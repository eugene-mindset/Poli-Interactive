...
<head>
  <title>Delete Bill Subject</title>
...
<body>
  <div class="container">
    <?php
      ...
      $stmt = $conn->prepare("CALL DeleteBillSubject(?,?,?)");
      $stmt->bind_param('sss', $bill, $cong, $sub);
      $stmt->execute();
      $result = $stmt->get_result();

      //
      if (!$result) {
        echo "<span class='err'>Call to DeleteBillSubject failed, entry does not exist.</span>";
        $stmt->close();
        $conn->close();
        return;
      }
